...
export const customOAuthBackend: OAuthConfig<UserModelOriginal> = {
  ...
  name: 'KL Site',
  type: 'oauth',
  authorization: `${env.get('NEXTAUTH_URL').required().asString()}/auth/login`,
  token: `${process.env.NEXT_PUBLIC_ACCOUNT_API_URL}/auth/token`,
  userinfo: `${process.env.NEXT_PUBLIC_ACCOUNT_API_URL}/auth/me`,
  clientId: env.get('NEXTAUTH_CLIENT_ID').required().asString(),
  clientSecret: env.get('NEXTAUTH_CLIENT_SECRET').required().asString(),
  httpOptions: {
